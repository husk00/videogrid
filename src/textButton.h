...
// da sistemare!!!!
//esto tiene que ser en relación de la width de la videogrid
    ...
    void checkMouseOver(ofPoint mouse){
    ...
    void checkMousePressed(ofPoint mouse){
        if (bSelectable == false){ bSelected = false; return;}
        if (rect.inside(mouse)){
            bSelected = true;
        } else {
            bSelected = false;
...
//                ofSetColor(220,220,220);
			...
			if( bSelectable ){
				if (bMouseOver == true && bSelected == false) ofSetColor(140,140,140);
				else ofSetColor(0,0,0);
				if (bSelected == true) ofSetColor(255,0,0);
				//else ofSetColor(0,0,0);
			}else{
